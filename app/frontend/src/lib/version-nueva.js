 ...
 * se abrio, pase lo que pase despues. Aqui las pestañas se quedan abiertas dias:
 * un panel de ventas no se cierra, se mira. Y una version vieja de un panel es de lo
 * peor que puede pasar, porque no se rompe nada — simplemente se decide sobre cifras
 * calculadas con codigo que ya se cambio, y nadie tiene motivo para dudar de ellas.
 *
 * QUE NO SIRVE PARA MEDIRLO. El `/api/health` del backend dice que version corre el
 * backend. Eso es OTRO contenedor, que se despliega por su cuenta: puede estar recien
 * actualizado con el front viejo, o al reves. Lo que hay que comparar es el JavaScript que esta
 ...
 * subir. index.html se sirve con `no-store` (esta en app/frontend/nginx.conf), asi que pedirlo devuelve siempre el de ahora — 2 kB.
 *
 ...
 */
const HUELLA_LOCAL = typeof document === "undefined" ? "" : huella(document.documentElement.outerHTML);
 ...
 *
 * Es el freno que hace imposible el bucle. En PEDIDO recargar solo salio mal dos veces —04/08/2026 y
 * 08/08/2026, la aplicacion parpadeando cada dos segundos en Sancti Spiritus— y las
 * dos por lo mismo: una recarga que no arregla la causa se repite para siempre. Si al volver seguimos viendo la misma version nueva por bajar
...
/**
 * Recarga trayendo el index.html nuevo de verdad, no el que el navegador tenga a mano.
 * Un `location.reload()` a secas puede devolver el MISMO documento —del bfcache o de la
 * memoria del navegador— y dejarlo todo igual.
 */
 ...
 */
export function vigilarVersion(alHaberVersionNueva) {
