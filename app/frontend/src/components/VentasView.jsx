...
export default function VentasView({ sid }) {
  const [data, setData] = useState(null);

  useEffect(() => {
    getVentas(sid).then(setData);
  }, [sid]);

  if (!data) return <div className="p-6">Cargando…</div>;

  const gestoresBar = data.gestores.map((g) => ({
    gestor: g.gestor,
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <a className="btn-primary" href={exportUrl(sid, "ventas")}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Kpi
          label="% Cumplimiento"
          value={`${formatNumber(data.cumplimiento_pct, 1)} %`}
      ...
      <BarCard title="Hectolitros vs Cuota por gestor" data={gestoresBar} xKey="gestor" yKey="hectolitros" />
                ...
                <tr key={g.gestor} className="border-t border-slate-100">
                  ...
                  <td
                    className={`px-3 py-2 text-right font-semibold ${
                      g.cumplimiento_pct >= 100
                        ? "text-emerald-600"
                        : g.cumplimiento_pct >= 80
                        ? "text-amber-600"
                        : "text-red-600"
                    }`}
                  >
